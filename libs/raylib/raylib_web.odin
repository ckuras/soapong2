...
when ODIN_OS == .Windows {
	when RAYLIB_SHARED {
		...
		foreign import lib {"windows/raylibdll.lib", "system:Winmm.lib", "system:Gdi32.lib", "system:User32.lib", "system:Shell32.lib"}
	} else {
	...
	when RAYLIB_SHARED {
		foreign import lib {"linux/libraylib.so.500", "system:dl", "system:pthread"}// Note(bumbread): I'm not sure why in `linux/` folder there are
		...
		foreign import lib {"linux/libraylib.a", "system:dl", "system:pthread"}
	}
} else when ODIN_OS == .Darwin {
	when ODIN_ARCH == .arm64 {
		when RAYLIB_SHARED {
			foreign import lib {"macos-arm64/libraylib.500.dylib", "system:Cocoa.framework", "system:OpenGL.framework", "system:IOKit.framework"}
		} else {
			foreign import lib {"macos-arm64/libraylib.a", "system:Cocoa.framework", "system:OpenGL.framework", "system:IOKit.framework"}
		}
	} else {
		when RAYLIB_SHARED {
			foreign import lib {"macos/libraylib.500.dylib", "system:Cocoa.framework", "system:OpenGL.framework", "system:IOKit.framework"}
		} else {
...
when VERSION == "5.0" {
	...
	#panic(
		"Remove this this when block and everything inside it for Raylib > 5.0. It's just here to fix a bug in Raylib 5.0. See IsMouseButtonUp inside 'foreign lib {' block.",
	)
}
...
TextFormat :: proc(text: cstring, args: ..any) -> cstring {
	@(static)
	buffers: [MAX_TEXTFORMAT_BUFFERS][MAX_TEXT_BUFFER_LENGTH]byte
	@(static)
	index: u32
